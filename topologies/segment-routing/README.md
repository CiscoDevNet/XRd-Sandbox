...
```bash
make -C ~/XRd-Sandbox fix-mgmt-interface-segment-routing
```

This updates `XR_MGMT_INTERFACES` in `docker-compose.yml`, replacing the counter-based interface name generated by `xr-compose` (e.g. `linux:xr-n120`) with `linux:eth0` — the actual interface name Docker assigns to `macvlan` networks inside the container. The `-C` flag lets you run the target from any working directory.
